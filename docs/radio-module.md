...
### Ограничения
- **Максимальное время прослушивания**: 30 минут (настраивается через `VITE_RADIO_MAX_PLAY_TIME_MINUTES`). Это мягкий лимит — при достижении радио останавливается, но пользователь может нажать Play и продолжить (таймер сбрасывается). `totalPlayedTime` периодически сохраняется в localStorage.
